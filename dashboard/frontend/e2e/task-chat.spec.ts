...
test.describe.serial('Task Chat', () => {
  ...
  test('shows conversation panel for awaiting task', async ({ adminPage }) => {
    await adminPage.goto(`/tasks/${TEST_IDS.tasks.awaiting}`);
    await expect(adminPage.getByText('Conversation')).toBeVisible();
  });
  ...
  test('viewer cannot see chat input', async ({ viewerPage }) => {
    await viewerPage.goto(`/tasks/${TEST_IDS.tasks.awaiting}`);
    await expect(viewerPage.getByPlaceholder('Send a follow-up message')).toHaveCount(0);
  });
