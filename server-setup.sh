...
if $FRESH_INSTALL; then
    ...
    # --- Existing NixOS: replace configuration.nix, keep hardware-configuration.nix ---
    # The server already has a hardware-configuration.nix with boot/disk/fs/network
    # settings. We replace configuration.nix with one that imports hardware-configuration.nix
    # plus tekton-services.nix for all the platform services.

    if [[ ! -f /etc/nixos/hardware-configuration.nix ]]; then
        fatal "Expected /etc/nixos/hardware-configuration.nix but it doesn't exist. Cannot proceed."
    fi

    # Prepare tekton-services.nix
    ...
    # Generate a new configuration.nix that imports hardware + tekton services
    info "Generating configuration.nix for existing NixOS server..."
    cat > "$TMPDIR/configuration.nix" << 'NIXEOF'
# Generated by tekton server-setup.sh
# Hardware/boot/network config comes from hardware-configuration.nix (auto-generated).
# Tekton platform services come from tekton-services.nix.
{ config, lib, pkgs, ... }:
{
  imports = [
